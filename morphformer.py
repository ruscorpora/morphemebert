...
# --- Вспомогательные классы (дублируются для автономности, либо вынести в utils) ---
...
class CharTransformerModel(nn.Module):
    def __init__(self, vocab_size, num_labels, d_model=256, nhead=4, num_layers=4, dim_feedforward=512, dropout=0.1):
        super().__init__()
        ...
        encoder_layers = nn.TransformerEncoderLayer(
            ...
            dropout=dropout,
            batch_first=True # Важно для input shape (Batch, Seq, Dim)
        )
    ...
    def forward(self, input_ids, labels=None):
        ...
        # 1. Create Padding Mask (True where value is PAD)
        # Shape: (Batch, Seq_Len)
...
# --- Обертка MorphTransformer ---

class MorphTransformer(MorphBase):
    ...
    def train(self, fold: int):
        ...
        model = CharTransformerModel(
            ...
            d_model=self.config.get("hidden_dim", 256),
        ...
        optimizer = torch.optim.AdamW(model.parameters(), lr=self.config.get("learning_rate", 1e-4)) # LR обычно ниже для трансформеров
    ...
    def predict(self, fold: int):
        ...
        # Переименуем параметр для обратной совместимости аргументов, если в конфиге сохранено hidden_dim
        # а конструктор ждет d_model (хотя я сделал d_model=hidden_dim в конфиге, но для чистоты)
        if "hidden_dim" in model_params and "d_model" not in model_params:
            model_params["d_model"] = model_params.pop("hidden_dim")

        model = CharTransformerModel(**model_params).to(self.device)
