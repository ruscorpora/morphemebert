...
class CharBiLSTMModel(nn.Module):
    def __init__(self, vocab_size, num_labels, embedding_dim=128, hidden_dim=256, num_layers=2, dropout=0.1):
        super().__init__()
        self.embedding = nn.Embedding(vocab_size, embedding_dim, padding_idx=0)
        self.dropout_emb = nn.Dropout(dropout)
        
        # BiLSTM: input_size=embedding_dim, bidirectional=True
        self.lstm = nn.LSTM(
        ...
        # Output layer: hidden_dim * 2 (потому что bidirectional)
...
class MorphBiLSTM(MorphBase):
    ...
    def predict(self, fold: int):
        ...
        model = CharBiLSTMModel(**model_params).to(self.device)
